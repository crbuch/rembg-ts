...
export class BaseSession {
    ...
    constructor(model_name: string, sess_opts: ort.InferenceSession.SessionOptions, ...args: unknown[]) {
        /**
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            const isUsingWebGPU = executionProviders.some(provider => 
                typeof provider === 'string' ? provider === 'webgpu' : provider.name === 'webgpu'
            );
            ...
            this.inner_session = await ort.InferenceSession.create(modelArrayBuffer, this.sess_opts);
            this.initialized = true;
            
            console.log(`Model ${this.model_name} loaded successfully`);
        } catch (error) {
            console.error(`Failed to initialize session for ${this.model_name}:`, error);
            throw error;
        }
    }
    ...
    async predict(_img: PILImage, ..._args: unknown[]): Promise<PILImage[]> {
        throw new Error("NotImplementedError: predict method must be implemented by subclass");
    }
    ...
    static getModelName(..._args: unknown[]): string {
        throw new Error("NotImplementedError: getModelName method must be implemented by subclass");
    }
}
 ...
 */
export const fixMaxPoolForWebGPU = async (modelBuffer: ArrayBuffer): Promise<ArrayBuffer> => {
  try {
    ...
    for (let i = 0; i < modifiedBytes.length - ceilModePattern.length - 10; i++) {
      ...
      if (patternFound) {
        ...
        for (let k = i + ceilModePattern.length; k < i + ceilModePattern.length + 20; k++) {
          if (k < modifiedBytes.length - 1) {
            if ((modifiedBytes[k] === 0x08 && modifiedBytes[k + 1] === 0x01) ||
    ...
    console.log(`Modified ${modificationsCount} MaxPool attributes for WebGPU compatibility`);
    return modifiedBytes.buffer;
    
  } catch (error) {
    console.warn("Failed to fix MaxPool operations, using original model:", error);
    return modelBuffer;
  }
};
